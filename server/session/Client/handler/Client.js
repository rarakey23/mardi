...
class Client {
	...
	async sendText(from, { message }) {
		const mentions = [...message.matchAll(/@(\d{0,16})/g)].map((v) => v[1] + "@s.whatsapp.net");
		return await this.client.sendMessage(from, { message, mentions });
	}

	async getName(number) {
		const contact = await this.client.getContact(number);
		return contact.name;
	}
	...
	async sendProduct(from, { title, image_url, message = "", footer = "", url, price, salePrice, currencyCode }) {
		let businessOwnerJid = url.split("/p/")[1].split("/")[1];
		let getIdProduct = url.split("/p/")[1].split("/")[0];
		let image = await prepareWAMessageMedia({ image: { url: image_url } }, { upload: this.client.waUploadToServer });
			...
			proto.Message.fromObject({
				productMessage: {
					product: {
						...
						footerText: "",
						priceAmount1000: price ? price : "0",
						productImageCount: 1,
						firstImageId: "123",
						salePriceAmount1000: salePrice ? salePrice : "0",
						retailerId: getIdProduct ? getIdProduct : "ilyasa",
						url: "",
					},
					footer,
					message,
